...
def test_duplicate_session_link(api_client, state: StateManager):
...
def test_invalid_update_body(api_client, state: StateManager):
    s = state.create_stream("Test", ["repo"])
    resp = api_client.post(f"/api/streams/{s.id}/updates", json={"summary": "missing content"})
    assert resp.status_code == 422


def test_invalid_decision_body(api_client, state: StateManager):
    s = state.create_stream("Test", ["repo"])
