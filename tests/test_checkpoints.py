...
class TestSaveCheckpoint:
    def test_creates_checkpoint(self, state: StateManager):
        cp = state.save_checkpoint("2026-03-02", "Week content")
        assert cp.week_of == "2026-03-02"
        assert cp.content == "Week content"

    def test_auto_captures_active_streams(self, state: StateManager):
    ...
    def test_explicit_stream_ids(self, state: StateManager):
    ...
    def test_upserts_on_same_week(self, state: StateManager):
...
class TestGetCheckpoint:
    def test_get_latest(self, state: StateManager):
    ...
    def test_get_specific_week(self, state: StateManager):
        state.save_checkpoint("2026-03-02", "Week 1")
        state.save_checkpoint("2026-03-09", "Week 2")
        cp = state.get_checkpoint("2026-03-02")
        assert cp.content == "Week 1"

    def test_returns_none_when_empty(self, state: StateManager):
        assert state.get_checkpoint() is None

    def test_returns_none_for_missing_week(self, state: StateManager):
        state.save_checkpoint("2026-03-02", "content")
        assert state.get_checkpoint("2099-01-01") is None


class TestCheckpointSearch:
    def test_checkpoint_found_in_search(self, state: StateManager):
    ...
    def test_checkpoint_not_found_for_unrelated_query(self, state: StateManager):
