...
def test_list_streams(api_client, state: MongoStateManager):
...
def test_list_streams_filter_completed(api_client, state: MongoStateManager):
...
def test_get_stream_context(api_client, state: MongoStateManager):
...
def test_patch_stream(api_client, state: MongoStateManager):
...
def test_delete_stream(api_client, state: MongoStateManager):
...
def test_complete_stream(api_client, state: MongoStateManager):
...
def test_create_update(api_client, state: MongoStateManager):
    s = state.create_stream("Test", ["repo"])
...
def test_patch_update(api_client, state: MongoStateManager):
    s = state.create_stream("Test", ["repo"])
...
def test_delete_update(api_client, state: MongoStateManager):
    s = state.create_stream("Test", ["repo"])
...
def test_create_decision(api_client, state: MongoStateManager):
    s = state.create_stream("Test", ["repo"])
    resp = api_client.post(
...
def test_patch_decision(api_client, state: MongoStateManager):
    s = state.create_stream("Test", ["repo"])
...
def test_delete_decision(api_client, state: MongoStateManager):
    s = state.create_stream("Test", ["repo"])
...
def test_link_session(api_client, state: MongoStateManager):
    s = state.create_stream("Test", ["repo"])
    resp = api_client.post(f"/api/streams/{s.id}/sessions", json={"session_id": "sess-001"})
    assert resp.status_code == 200


def test_unlink_session(api_client, state: MongoStateManager):
    s = state.create_stream("Test", ["repo"])
...
def test_move_session(api_client, state: MongoStateManager):
...
def test_activity(api_client, state: MongoStateManager):
    s = state.create_stream("Test", ["repo"])
    state.add_update(s.id, "content", "summary")
    state.add_decision(s.id, "what", "why")
...
def test_search(api_client, state: MongoStateManager):
    s = state.create_stream("Test", ["repo"])
