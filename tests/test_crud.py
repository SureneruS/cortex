...
class TestEditUpdate:
    def test_edit_content(self, state: MongoStateManager):
    ...
    def test_edit_summary(self, state: MongoStateManager):
        s = state.create_stream("Test", ["repo"])
    ...
    def test_edit_metadata(self, state: MongoStateManager):
        s = state.create_stream("Test", ["repo"])
        u = state.add_update(s.id, "Content", "Summary", metadata={"a": 1})
        edited = state.edit_update(u.id, metadata={"b": 2})
        assert edited.metadata == {"b": 2}

    def test_edit_nonexistent_returns_none(self, state: MongoStateManager):
        assert state.edit_update("nonexistent") is None

    def test_no_changes_returns_original(self, state: MongoStateManager):
        s = state.create_stream("Test", ["repo"])
        u = state.add_update(s.id, "Content", "Summary")
        result = state.edit_update(u.id)
        assert result.content == "Content"

    def test_edit_updates_search_index(self, state: MongoStateManager):
        s = state.create_stream("Test", ["repo"])
...
class TestEditDecision:
    def test_edit_what(self, state: MongoStateManager):
        s = state.create_stream("Test", ["repo"])
    ...
    def test_edit_why(self, state: MongoStateManager):
        s = state.create_stream("Test", ["repo"])
...
class TestDeleteStream:
    def test_deletes_stream(self, state: MongoStateManager):
        s = state.create_stream("Test", ["repo"])
        state.delete_stream(s.id)
        assert state.get_stream(s.id) is None

    def test_cascades_updates(self, state: MongoStateManager):
        s = state.create_stream("Test", ["repo"])
    ...
    def test_cascades_decisions(self, state: MongoStateManager):
        s = state.create_stream("Test", ["repo"])
    ...
    def test_deindexes_entries(self, state: MongoStateManager):
        s = state.create_stream("Test", ["repo"])
        state.add_update(s.id, "unique_xyzzy_token", "unique_xyzzy_token")
        state.delete_stream(s.id)
        assert len(state.search("unique_xyzzy_token")) == 0

    def test_delete_nonexistent_is_safe(self, state: MongoStateManager):
