...
class TestVecIndexLifecycle:
    def test_vec_index_populated_on_add_update(self, state: StateManager):
    ...
    def test_vec_index_populated_on_add_decision(self, state: StateManager):
        stream = state.create_stream("Test", ["repo"])
        state.add_decision(stream.id, "Use WAL mode", "Better concurrency")
        count = state._conn.execute("SELECT COUNT(*) FROM vec_map").fetchone()[0]
        assert count == 1

    def test_vec_deindex_on_delete_update(self, state: StateManager):
        stream = state.create_stream("Test", ["repo"])
    ...
    def test_vec_deindex_on_delete_decision(self, state: StateManager):
        stream = state.create_stream("Test", ["repo"])
    ...
    def test_edit_update_reindexes_vec(self, state: StateManager):
        stream = state.create_stream("Test", ["repo"])
...
class TestVecSearch:
    def test_vec_search_returns_results(self, populated_state: StateManager):
        results = populated_state._vec_search("docker container setup")
        assert len(results) >= 1
        assert all(len(r) == 3 for r in results)

    def test_semantic_search_finds_content(self, populated_state: StateManager):
        results = populated_state.search("docker")
        assert len(results) >= 1

    def test_search_returns_typed_entities(self, populated_state: StateManager):
        results = populated_state.search("WAL concurrent")
        assert any(isinstance(r, (Update, Decision)) for r in results)


class TestReindex:
    def test_reindex_populates_from_scratch(self, populated_state: StateManager):
    ...
    def test_clear_indexes_empties_vec(self, populated_state: StateManager):
        populated_state.clear_indexes()
        vec_count = populated_state._conn.execute("SELECT COUNT(*) FROM vec_map").fetchone()[0]
        assert vec_count == 0

    def test_clear_indexes_empties_fts_when_available(self, populated_state: StateManager):
        if not populated_state._has_fts:
            pytest.skip("FTS5 not available")
        populated_state.clear_indexes()
        fts_count = populated_state._conn.execute("SELECT COUNT(*) FROM search_index").fetchone()[0]
        assert fts_count == 0


class TestHybridFallback:
    def test_fts_fallback_for_exact_keyword(self, state: StateManager):
        stream = state.create_stream("Test", ["repo"])
